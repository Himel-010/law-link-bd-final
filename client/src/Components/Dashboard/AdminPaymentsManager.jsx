...
const planNameOptions = ["all", "free", "basic", "premium"];
...
const MethodBadge = ({ method }) => (
  <span
    className={`inline-flex rounded-full border px-3 py-1 text-xs font-black uppercase ${getMethodClasses(
      method
    )}`}
  >
    {method || "-"}
...
const StatCard = ({ label, value, tone = "slate" }) => {
  ...
  return (
    <div className={`rounded-3xl border bg-white p-5 shadow-sm ${toneClass[tone]}`}>
...
export default function AdminPaymentsManager() {
  ...
  }, [page, limit, paymentStatus, method, roleType, planName]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-cyan-50/60 via-slate-50 to-white p-4 pt-24 sm:p-6 sm:pt-24">
      <div className="mx-auto max-w-7xl">
        ...
        >
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
              Review manual bKash/Nogod purchases, verify valid payments, reject
              invalid submissions, and refund verified payments.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 rounded-full border border-cyan-100 bg-white/80 px-4 py-3 shadow-sm">
            <div className="flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-cyan-500 to-cyan-700 text-sm font-black text-white">
              {(authUser?.name || authUser?.email || "A").slice(0, 1).toUpperCase()}
            </div>
            <div>
        ...
        <div className="mb-8 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          ...
          <StatCard label="Refunded" value={stats.refunded} tone="blue" />
        </div>
        ...
        <div className="mb-8 rounded-3xl border border-cyan-100 bg-white/90 p-5 shadow-xl backdrop-blur">
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                Search transaction ID, user, sender number, or plan name.
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-6">
            ...
            <select
              value={planName}
              onChange={handleFilterChange(setPlanName)}
              className="rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-800 outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500"
            >
              {planNameOptions.map((item) => (
                <option key={item} value={item}>
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-cyan-500 to-cyan-700 px-5 py-3 text-sm font-black text-white shadow-lg shadow-cyan-500/20 transition hover:from-cyan-600 hover:to-cyan-800"
            >
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center p-12 text-center">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Try changing filters or check if users have submitted payment requests.
              </p>
            </div>
          ) : (
        ...
        {selectedPayment && (
          <div className="mt-8 overflow-hidden rounded-3xl border border-cyan-100 bg-white shadow-xl">
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-cyan-100 bg-gradient-to-r from-cyan-50 to-white px-5 py-5">
              <div>
                <h2 className="text-xl font-black text-slate-950">
                  Payment Details
                </h2>
                <p className="mt-1 text-sm text-slate-500">
                  Full payment, user, plan, and verification details in table format.
                </p>
              </div>
            ...
            {detailLoading ? (
              ...
              <div className="overflow-x-auto p-5">
                <table className="min-w-[900px] w-full border-separate border-spacing-0 overflow-hidden rounded-2xl border border-slate-200">
                  <tbody className="divide-y divide-slate-200">
                    ...
                    <DetailRow
                      label="Sender Number"
                      value={selectedPayment.senderNumber}
                    />
                    <DetailRow
                      label="Payment Date"
                    ...
                    <DetailRow
                      label="User Phone"
                      value={selectedPayment.user?.phone || "-"}
                    />
                    <DetailRow
                      label="Role Type"
                      value={selectedPayment.roleType}
                    />
                    <DetailRow
                      label="Plan Name"
                    ...
                    <DetailRow
                      label="Plan Slug"
                      value={selectedPayment.planSlug}
                    />
                    <DetailRow
                      label="Verified By"
                    ...
                    />
                    <DetailRow label="Note" value={selectedPayment.note || "-"} />
                  </tbody>
                </table>
              </div>
            )}
      ...
      {actionModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-3xl border border-cyan-100 bg-white shadow-2xl">
